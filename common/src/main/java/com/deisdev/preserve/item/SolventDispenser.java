...
public final class SolventDispenser implements DispenseItemBehavior {
    public static void register() { DispenserBlock.registerBehavior(PreserveItems.RELEASE_SOLVENT.get(), new SolventDispenser()); }
    @Override public ItemStack dispense(BlockSource source, ItemStack stack) {
        ...
        if (result.changed()) {
            ...
            } catch (RuntimeException error) { com.deisdev.preserve.Constants.LOG.warn("Solvent removal committed, but feedback failed", error); }
        }
