...
public final class TransferPolicyMenu extends AbstractContainerMenu {
    ...
    public TransferPolicyMenu(int id, Inventory inventory, int sourceSlot) {
        super(PreserveMenus.TRANSFER_POLICY.get(), id);
    ...
    private void refresh() {
        ...
        var policy = jar.getOrDefault(PreserveItems.TRANSFER_POLICY.get(), TransferPolicy.DEFAULT);
    ...
    @Override public boolean stillValid(Player player) {
        return sourceSlot < 0 || player == inventory.player && player.isAlive() && !player.isSpectator()
                && inventory.getItem(sourceSlot) == jar && PreserveItems.TRANSFER_SEAL.get().remaining(jar) > 0;
    }
    @Override public boolean clickMenuButton(Player player, int button) {
        ...
        jar.set(PreserveItems.TRANSFER_POLICY.get(), next); inventory.setChanged(); broadcastChanges(); return true;
    }
