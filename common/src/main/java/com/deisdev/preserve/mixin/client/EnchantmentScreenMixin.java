...
public abstract class EnchantmentScreenMixin extends AbstractContainerScreen<EnchantmentMenu> {
    ...
    private int preserve$affordableSerum(int actual) {
        return menu.getSlot(0).getItem().is(PreserveItems.TIME_SERUM.get()) ? (actual >= ((SerumMenu) menu).preserve$lapisCost() ? 3 : 0) : actual;
    }
    ...
    private void preserve$serumCosts(GuiGraphicsExtractor graphics, Font font, List<Component> lines, int x, int y, Operation<Void> original) {
        if (menu.getSlot(0).getItem().is(PreserveItems.TIME_SERUM.get())) {
            var costs = (SerumMenu) menu;
            lines = List.of(Component.translatable("item.deisdev.suspicious_time_serum"),
                    Component.translatable("container.enchant.level.requirement", menu.costs[2]),
                    Component.translatable("container.enchant.lapis.many", costs.preserve$lapisCost()),
                    Component.translatable("container.enchant.level.many", costs.preserve$levelsSpent()));
        }
