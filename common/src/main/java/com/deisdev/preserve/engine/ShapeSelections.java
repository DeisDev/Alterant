...
final class ShapeSelections {
    static final class Selection {
        ...
        boolean matches(ServerPlayer player, BlockPos pos) {
            return valid && tool.get() == player.getMainHandItem() && preview.equals(player.getMainHandItem().get(PreserveItems.SHAPE_PREVIEW.get()))
    ...
    void put(ServerPlayer player, ShapePreview preview) {
        ...
        player.getMainHandItem().set(PreserveItems.SHAPE_PREVIEW.get(), preview); player.getInventory().setChanged();
    }
    void clear(ServerPlayer player) { var selected = players.remove(player); if (selected != null) { selected.valid = false; } player.getMainHandItem().remove(PreserveItems.SHAPE_PREVIEW.get()); player.getInventory().setChanged(); }
