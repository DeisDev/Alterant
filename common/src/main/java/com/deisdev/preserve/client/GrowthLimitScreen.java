...
import com.deisdev.preserve.item.GrowthLimitMenu;
...
public final class GrowthLimitScreen extends AbstractContainerScreen<GrowthLimitMenu> {
    ...
    @Override protected void init() {
        ...
        for (int mode = 0; mode < 2; mode++) {
            ...
            addRenderableWidget(Button.builder(Component.empty(), ignored -> send(button)).bounds(leftPos + 8 + mode * 26, topPos + 20, 22, 22)
                    .tooltip(Tooltip.create(Component.translatable(mode == 0 ? "menu.deisdev.growth.stage" : "menu.deisdev.growth.height"))).build());
        }
    ...
    private void updateChoices() {
        for (int index = 0; index < choices.size(); index++) {
            ...
            button.setTooltip(Tooltip.create(Component.translatable("item.deisdev.growth_regulator." + menu.mode().getSerializedName(), value(index))));
        }
    }
    ...
    @Override protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        graphics.text(font, Component.translatable("item.deisdev.growth_regulator." + menu.mode().getSerializedName(), menu.target()), 65, 27, 0xFF454033, false);
