...
import com.deisdev.preserve.engine.TransferPolicy;
import com.deisdev.preserve.item.TransferPolicyMenu;
...
public final class TransferPolicyScreen extends AbstractContainerScreen<TransferPolicyMenu> {
    ...
    @Override protected void init() {
        ...
        for (int index = 0; index < 3; index++) {
            int id = index;
            modes[index] = addRenderableWidget(Button.builder(Component.translatable("menu.deisdev.transfer." + TransferPolicy.Mode.values()[index].getSerializedName()), ignored -> send(id))
                    .bounds(leftPos + 8 + index * 54, topPos + 20, 52, 20)
                    .tooltip(Tooltip.create(Component.translatable("item.deisdev.transfer_seal." + TransferPolicy.Mode.values()[index].getSerializedName()))).build());
        }
        for (var face : Direction.values()) {
            int id = face.ordinal();
            faces[id] = addRenderableWidget(Button.builder(Component.translatable("menu.deisdev.transfer.face." + face.getSerializedName()), ignored -> send(10 + id))
                    .bounds(leftPos + FACE_POS[id][0], topPos + FACE_POS[id][1], 22, 20)
                    .tooltip(Tooltip.create(Component.translatable("menu.deisdev.transfer.seal_face", Component.translatable("menu.deisdev.transfer.direction." + face.getSerializedName())))).build());
        }
        addRenderableWidget(Button.builder(Component.translatable("menu.deisdev.transfer.all"), ignored -> send(20)).bounds(leftPos + 132, topPos + 94, 36, 20).build());
        updateChoices();
    }
    ...
    private void updateChoices() {
        ...
        for (var face : Direction.values()) {
            ...
            faces[face.ordinal()].setMessage(Component.translatable("menu.deisdev.transfer.face." + face.getSerializedName())
