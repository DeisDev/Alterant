...
public final class TemporalRecipe extends NormalCraftingRecipe {
    ...
    @Override protected PlacementInfo createPlacementInfo() {
        ...
        BASES.forEach(formulation -> ingredients.add(Ingredient.of(PreserveItems.compound(formulation))));
        ingredients.addAll(additives);
        return PlacementInfo.create(ingredients);
    }
    @Override public boolean matches(CraftingInput input, Level level) { return valid(input); }
    @Override public ItemStack assemble(CraftingInput input) { return valid(input) ? PreserveItems.TEMPORAL_STASIS.get().getDefaultInstance() : ItemStack.EMPTY; }
    ...
    @Override public List<RecipeDisplay> display() {
        ...
        for (var formulation : BASES) {
            SlotDisplay jar = new SlotDisplay.ItemStackSlotDisplay(ItemStackTemplate.fromNonEmptyStack(PreserveItems.compound(formulation).getDefaultInstance()));
        ...
        return List.of(new ShapelessCraftingRecipeDisplay(inputs,
                new SlotDisplay.ItemStackSlotDisplay(ItemStackTemplate.fromNonEmptyStack(PreserveItems.TEMPORAL_STASIS.get().getDefaultInstance())),
