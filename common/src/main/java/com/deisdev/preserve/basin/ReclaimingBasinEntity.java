...
public final class ReclaimingBasinEntity extends BaseContainerBlockEntity implements WorldlyContainer {
    ...
    public ReclaimingBasinEntity(BlockPos pos, BlockState state) { super(PreserveBlocks.BASIN_ENTITY.get(), pos, state); }
    ...
    @Override protected Component getDefaultName() { return Component.translatable("block.deisdev.reclaiming_basin"); }
    ...
    public void tick(ServerLevel server) {
        ...
        if (selected == null) {
            ...
            looseFamily(items.get(0)).ifPresent(family -> selected = ResourceKey.create(Registries.RECIPE,
                    Identifier.fromNamespaceAndPath("deisdev", "reclaim/" + family.getSerializedName())));
