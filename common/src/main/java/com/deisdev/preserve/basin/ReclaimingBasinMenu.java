...
import com.deisdev.preserve.item.*;
import java.util.*;
...
public final class ReclaimingBasinMenu extends AbstractContainerMenu {
    ...
    public ReclaimingBasinMenu(int id, Inventory playerInventory, Container inventory) {
        super(PreserveMenus.BASIN.get(), id); this.inventory = inventory;
    ...
    private void refresh() {
        ...
        for (int icon = 0; icon < 4; icon++) {
            ...
            if (index < choices.size()) {
                ...
                lore.add(Component.translatable("menu.deisdev.basin.residue", recipe.count(), Component.translatable("item.deisdev." + recipe.family().getSerializedName())));
                if (recipe.additive().isPresent()) {
                    ...
                    lore.add(Component.translatable("menu.deisdev.basin.additive", recipe.additiveCount(), names));
                }
                lore.add(Component.translatable("menu.deisdev.basin.duration", recipe.duration() / 20.0));
