...
public final class RuleRegistry {
    ...
    public static CompiledRules get(net.minecraft.server.MinecraftServer server) {
        var registry = ((PreservationServer) server).alterant$rules();
        if (registry.settingsFile == null) {
            registry.settingsFile = new GameplaySettingsFile(server.getWorldPath(net.minecraft.world.level.storage.LevelResource.ROOT)
                    .resolve("serverconfig/alterant.json"));
        }
        return registry.accept(com.deisdev.alterant.platform.Services.PLATFORM.loadedRules(server));
    }

    public CompiledRules accept(RuleLoad load) {
        if (load == null) { throw new IllegalStateException("Alterant rules were not loaded; check loader registration"); }
        if (load != seen) {
            seen = load;
            error = load.error();
            if (load.rules() != null) {
            ...
            if (!error.isEmpty() && current != null) { Constants.LOG.error("Alterant kept its previous rules: {}", error); }
        }
        if (current == null) { throw new IllegalStateException("Cannot load initial Alterant rules: " + error); }
        return current;
    }
