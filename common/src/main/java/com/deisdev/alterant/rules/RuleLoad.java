...
public record RuleLoad(CompiledRules rules, String error) {
    public static final Identifier ID = Identifier.parse("alterant:rules");
    public static final Identifier SETTINGS = Identifier.parse("alterant:alterant/settings.json");

    public RuleLoad requireValid() {
        if (rules == null) { throw new IllegalStateException("Invalid Alterant data pack rules: " + error); }
        return this;
    }

    public static RuleLoad prepare(ResourceManager manager, HolderLookup.Provider registries, Predicate<String> modLoaded) {
        try {
            var converter = FileToIdConverter.json("alterant/rules");
            var files = converter.listMatchingResources(manager);
            if (files.size() > 1024) { throw new IllegalArgumentException("Too many Alterant rule files"); }
            ...
            if (definitions.stream().noneMatch(rule -> rule.id().equals(Identifier.parse("alterant:standard_ticks")))) {
                throw new IllegalArgumentException("Required rule alterant:standard_ticks is missing; restore it or explicitly disable formulations in settings");
            }
    ...
    private static <T> T read(Resource resource, Codec<T> codec) throws java.io.IOException {
        try (Reader reader = resource.openAsReader()) {
            ...
            while ((count = reader.read(buffer)) != -1) {
                if (text.length() + count > 65536) { throw new IllegalArgumentException("Alterant JSON file exceeds 64 KiB"); }
