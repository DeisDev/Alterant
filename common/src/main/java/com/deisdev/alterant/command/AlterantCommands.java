    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("alterant")
                ...
                .then(Commands.literal("prepare-uninstall")
                        .executes(context -> {
                            context.getSource().sendSuccess(() -> Component.literal("Back up the world first. Then run /alterant prepare-uninstall start. Cleanup loads treated areas and their neighbors across every dimension; use status or cancel to manage it. /alterant thaw only removes a loaded target."), false);
                            return 1;
                        })
                        .then(Commands.literal("start").executes(context -> {
                            ...
                            source.sendSuccess(() -> Component.literal(job.status() + " Use /alterant prepare-uninstall status to check progress."), false);
                            return started ? 1 : 0;
                        }))
