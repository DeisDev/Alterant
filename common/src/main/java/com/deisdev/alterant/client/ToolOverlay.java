...
public final class ToolOverlay {
    ...
    public static List<Mark> extract(ClientLevel level) {
        ...
        var store = ((PreservationLevel) level).alterant$treatments();
        ...
        if (settings.surfacePreview() && (PreservingBrushItem.area(client.player.getMainHandItem()) || com.deisdev.alterant.item.ReleaseSolventItem.area(client.player.getMainHandItem())) && client.hitResult instanceof BlockHitResult hit && hit.getType() == HitResult.Type.BLOCK
