...
public final class SerumCard {
    ...
    public record Card(Vec3 anchor, List<Line> lines, int width, int height, ClientConfig.SerumDisplay display) {
        public Card { lines = List.copyOf(lines); }
    }
    private SerumCard() {}

    public static Card layout(Font font, Vec3 anchor, Component title, Component detail, int accent, ClientConfig.SerumDisplay display) {
        ...
        return new Card(anchor, lines, lines.stream().mapToInt(Line::width).max().orElse(0) + 16, lines.size() * 11 + 10, display);
    }

    public static void submit(LevelRenderState state, PoseStack pose, SubmitNodeCollector collector) {
        var card = ((OverlayRenderState) state).alterant$serumCard();
        if (card == null) { return; }
        ...
        pose.translate(card.display.x() / 100.0, 0, 0);
        var style = card.display.style();
        float scale = 0.01F * style.factor();
        ...
        if (style.opacity() > 0) { collector.submitCustomGeometry(pose, RenderTypes.textBackground(), (matrix, buffer) -> {
            // Small clipped corners soften the silhouette without a border or accent strip.
            quad(matrix, buffer, 2, 0, card.width - 2, card.height, style.backgroundArgb());
            quad(matrix, buffer, 0, 2, 2, card.height - 2, style.backgroundArgb());
            quad(matrix, buffer, card.width - 2, 2, card.width, card.height - 2, style.backgroundArgb());
        }); }
        var text = collector.order(1);
        for (int i = 0; i < card.lines.size(); i++) {
            var line = card.lines.get(i);
            text.submitText(pose, (card.width - line.width) / 2.0F, 6 + i * 11, line.text, style.shadow(),
                    Font.DisplayMode.POLYGON_OFFSET, LightCoordsUtil.FULL_BRIGHT, line.color, 0, 0);
        }
        pose.popPose();
    }

    private static void quad(PoseStack.Pose matrix, com.mojang.blaze3d.vertex.VertexConsumer buffer, float left, float top, float right, float bottom, int color) {
