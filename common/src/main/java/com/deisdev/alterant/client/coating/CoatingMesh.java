...
public record CoatingMesh(Identifier sourceAtlas, List<Vertex> vertices) {
    ...
    public record Vertex(float x, float y, float z, float u, float v, int sourceU, int sourceV,
    ...
    public void draw(PoseStack.Pose pose, VertexConsumer output) {
        for (var v : vertices) {
            output.addVertex(pose.pose(), v.x, v.y, v.z).setColor(v.color).setUv(v.u, v.v)
