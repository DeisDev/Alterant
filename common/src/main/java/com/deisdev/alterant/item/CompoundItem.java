...
public final class CompoundItem extends Item {
    ...
    @Override public ItemStack getDefaultInstance() {
        ...
        stack.set(AlterantItems.JAR_CONTENTS.get(), JarContents.full(formulation));
        return stack;
    }
    public int remaining(ItemStack stack) {
        var contents = stack.get(AlterantItems.JAR_CONTENTS.get());
    ...
    public ItemStack afterUse(ItemStack stack, int uses) {
        ...
        var next = stack.get(AlterantItems.JAR_CONTENTS.get()).spend(formulation, uses);
        if (next.remaining() == 0) { return new ItemStack(Items.GLASS_BOTTLE); }
        var result = stack.copy();
        result.set(AlterantItems.JAR_CONTENTS.get(), next);
        return result;
    }
    ...
    @Override public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display, Consumer<Component> lines, TooltipFlag flag) {
        if (!display.shows(AlterantItems.JAR_CONTENTS.get())) { return; }
        lines.accept(Component.translatable("item.alterant.jar.uses", remaining(stack), formulation.capacity()).withStyle(ChatFormatting.GRAY));
        if (remaining(stack) == 0) { lines.accept(Component.translatable("item.alterant.jar.empty").withStyle(ChatFormatting.RED)); }
        else { lines.accept(Component.translatable(formulation.accelerates() ? "item.alterant.serum.applicator" : "item.alterant.jar.brush").withStyle(ChatFormatting.DARK_GRAY)); }
        if (formulation.accelerates()) { SerumTooltip.append(formulation, lines); }
        if (formulation == Formulation.TRANSFER_SEAL) {
            var policy = stack.getOrDefault(AlterantItems.TRANSFER_POLICY.get(), com.deisdev.alterant.engine.TransferPolicy.DEFAULT);
            lines.accept(Component.translatable("item.alterant.transfer_seal." + policy.mode().getSerializedName()).withStyle(ChatFormatting.GRAY));
            lines.accept(Component.translatable("item.alterant.transfer_seal.faces", Integer.bitCount(policy.faces())).withStyle(ChatFormatting.GRAY));
            lines.accept(Component.translatable("item.alterant.transfer_seal.configure").withStyle(ChatFormatting.DARK_GRAY));
        }
        if (formulation == Formulation.GROWTH_REGULATOR) {
            var limit = stack.getOrDefault(AlterantItems.GROWTH_LIMIT.get(), com.deisdev.alterant.engine.GrowthLimit.DEFAULT);
            lines.accept(Component.translatable("item.alterant.growth_regulator." + limit.mode().getSerializedName(), limit.target()).withStyle(ChatFormatting.GRAY));
            lines.accept(Component.translatable("item.alterant.growth_regulator.configure").withStyle(ChatFormatting.DARK_GRAY));
        }
    }
