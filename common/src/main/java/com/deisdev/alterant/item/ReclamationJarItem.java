...
public final class ReclamationJarItem extends Item {
    ...
    public static boolean valid(ItemStack stack) { return stack.is(AlterantItems.RECLAMATION_JAR.get()) && stack.getCount() == 1; }
    public static ReclamationContents contents(ItemStack stack) { return valid(stack) ? stack.getOrDefault(AlterantItems.RECLAMATION_CONTENTS.get(), ReclamationContents.EMPTY) : ReclamationContents.EMPTY; }
    public static void setContents(ItemStack stack, ReclamationContents contents) {
        ...
        if (contents.total() == 0) { stack.remove(AlterantItems.RECLAMATION_CONTENTS.get()); }
        else { stack.set(AlterantItems.RECLAMATION_CONTENTS.get(), contents); }
    }
    ...
    @Override public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display, Consumer<Component> lines, TooltipFlag flag) {
        ...
        lines.accept(Component.translatable("item.alterant.reclamation_jar.contents", contents.total()).withStyle(ChatFormatting.GRAY));
        lines.accept(Component.translatable("item.alterant.reclamation_jar.use").withStyle(ChatFormatting.DARK_GRAY));
        if (flag.isAdvanced()) { for (var family : ResidueFamily.values()) {
            lines.accept(Component.translatable("item.alterant.reclamation_jar.family", Component.translatable("item.alterant." + family.getSerializedName()), contents.count(family)).withStyle(ChatFormatting.GRAY));
        } }
    }
}
