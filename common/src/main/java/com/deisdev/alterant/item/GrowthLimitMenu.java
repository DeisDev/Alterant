...
public final class GrowthLimitMenu extends AbstractContainerMenu {
    ...
    public GrowthLimitMenu(int id, Inventory inventory, int sourceSlot) {
        super(AlterantMenus.GROWTH_LIMIT.get(), id);
    ...
    private void refresh() {
        ...
        var limit = jar.getOrDefault(AlterantItems.GROWTH_LIMIT.get(), GrowthLimit.DEFAULT);
    ...
    @Override public boolean stillValid(Player player) {
        return sourceSlot < 0 || player == inventory.player && player.isAlive() && !player.isSpectator()
                && inventory.getItem(sourceSlot) == jar && AlterantItems.GROWTH_REGULATOR.get().remaining(jar) > 0;
    }
    @Override public boolean clickMenuButton(Player player, int button) {
        ...
        jar.set(AlterantItems.GROWTH_LIMIT.get(), limit); inventory.setChanged(); refresh(); broadcastChanges(); return true;
    }
