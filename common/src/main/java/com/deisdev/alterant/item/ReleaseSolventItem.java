...
public class ReleaseSolventItem extends Item implements PreservationTool {
    ...
    @Override public ItemStack getDefaultInstance() {
        var stack = new ItemStack(this); stack.set(AlterantItems.SOLVENT_DOSES.get(), CAPACITY); return stack;
    }
    public static int remaining(ItemStack stack) {
        int doses = stack.getOrDefault(AlterantItems.SOLVENT_DOSES.get(), 0);
    ...
    public static ItemStack afterUse(ItemStack stack, int doses) {
        ...
        var next = stack.copy(); next.set(AlterantItems.SOLVENT_DOSES.get(), remaining - doses); return next;
    }
    public static boolean area(ItemStack stack) { return stack.getItem() instanceof ReleaseSolventItem && stack.getOrDefault(AlterantItems.SOLVENT_AREA.get(), false); }
    @Override public InteractionResult use(net.minecraft.world.level.Level level, net.minecraft.world.entity.player.Player player, InteractionHand hand) {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            var stack = serverPlayer.getMainHandItem(); boolean area = !area(stack); stack.set(AlterantItems.SOLVENT_AREA.get(), area);
            serverPlayer.getInventory().setChanged();
            serverPlayer.sendOverlayMessage(Component.translatable(area ? "item.alterant.release_solvent.area" : "item.alterant.release_solvent.single"));
        }
    ...
    @Override public InteractionResult useOn(UseOnContext context) {
        ...
        var result = area(player.getMainHandItem()) ? com.deisdev.alterant.engine.SurfaceRelease.apply(player, context.getClickedPos(), context.getClickedFace())
    ...
    @Override public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display, Consumer<Component> lines, TooltipFlag flag) {
        lines.accept(Component.translatable("item.alterant.jar.uses", remaining(stack), CAPACITY).withStyle(ChatFormatting.GRAY));
        lines.accept(Component.translatable("item.alterant.release_solvent.use").withStyle(ChatFormatting.DARK_GRAY));
        lines.accept(Component.translatable(area(stack) ? "item.alterant.release_solvent.area" : "item.alterant.release_solvent.single").withStyle(ChatFormatting.GRAY));
        lines.accept(Component.translatable("item.alterant.release_solvent.mode").withStyle(ChatFormatting.DARK_GRAY));
    }
}
