...
public class ShapingStylusItem extends Item implements PreservationTool {
    ...
    public static int mode(ItemStack stack) { return stack.getOrDefault(AlterantItems.STYLUS_MODE.get(), 0); }
    @Override public InteractionResult use(net.minecraft.world.level.Level level, net.minecraft.world.entity.player.Player player, InteractionHand hand) {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            ...
            var tool = player.getMainHandItem(); int next = (mode(tool) + 1) % 3; tool.set(AlterantItems.STYLUS_MODE.get(), next);
            player.getInventory().setChanged(); serverPlayer.sendOverlayMessage(Component.translatable("item.alterant.shaping_stylus.mode." + next));
        }
    ...
    @Override public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display, Consumer<Component> lines, TooltipFlag flag) {
        lines.accept(Component.translatable("item.alterant.shaping_stylus.mode." + mode(stack)).withStyle(ChatFormatting.GRAY));
        lines.accept(Component.translatable("item.alterant.shaping_stylus.use").withStyle(ChatFormatting.GRAY));
        lines.accept(Component.translatable("item.alterant.shaping_stylus.payment").withStyle(ChatFormatting.DARK_GRAY));
        lines.accept(Component.translatable("item.alterant.shaping_stylus.modes").withStyle(ChatFormatting.DARK_GRAY));
        var sample = stack.get(AlterantItems.SHAPE_SAMPLE.get());
        if (sample != null) { lines.accept(Component.translatable("item.alterant.shaping_stylus.sample", Component.translatable("item.alterant.shaping_stylus." + sample.kind().getSerializedName())).withStyle(ChatFormatting.DARK_GRAY)); }
    }
}
