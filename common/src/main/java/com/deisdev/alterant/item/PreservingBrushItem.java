...
public final class PreservingBrushItem extends Item implements PreservationTool {
    ...
    public static boolean area(ItemStack stack) { return stack.getItem() instanceof PreservingBrushItem && stack.getOrDefault(AlterantItems.BRUSH_AREA.get(), false); }
    @Override public InteractionResult use(net.minecraft.world.level.Level level, net.minecraft.world.entity.player.Player player, InteractionHand hand) {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            ...
            stack.set(AlterantItems.BRUSH_AREA.get(), area);
            serverPlayer.getInventory().setChanged();
            serverPlayer.sendOverlayMessage(Component.translatable(area ? "item.alterant.preserving_brush.area" : "item.alterant.preserving_brush.single"));
        }
    ...
    @Override public InteractionResult useOn(UseOnContext context) {
        ...
        var result = area(player.getMainHandItem())
                ? com.deisdev.alterant.engine.SurfaceApplication.apply(player, context.getClickedPos(), context.getClickedFace(), context.isSecondaryUseActive())
    ...
    @Override public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display, Consumer<Component> lines, TooltipFlag flag) {
        lines.accept(Component.translatable("item.alterant.preserving_brush.use").withStyle(ChatFormatting.GRAY));
        lines.accept(Component.translatable(area(stack) ? "item.alterant.preserving_brush.area" : "item.alterant.preserving_brush.single").withStyle(ChatFormatting.GRAY));
        lines.accept(Component.translatable("item.alterant.preserving_brush.mode").withStyle(ChatFormatting.DARK_GRAY));
        lines.accept(Component.translatable("item.alterant.preserving_brush.replace").withStyle(ChatFormatting.DARK_GRAY));
    }
}
