...
public final class SerumTooltip {
    ...
    private static void append(Formulation formulation, TimeSettings time, Consumer<Component> lines) {
        var strength = formulation == Formulation.SUSPICIOUS_TIME_SERUM
                ? Component.translatable("item.alterant.serum.random_strength", number(time.suspicious().minimum()), number(time.suspicious().maximum()))
                : Component.translatable("item.alterant.serum.strength", number(time.profile(formulation).multiplier()));
        ...
        lines.accept(Component.translatable("item.alterant.serum.duration", minutes).withStyle(ChatFormatting.GRAY));
    }
