...
public enum ResidueFamily implements StringRepresentable {
    ...
    public Item item() {
        return switch (this) {
            case DRIED_COMPOUND -> AlterantItems.DRIED_COMPOUND.get();
            case SEALANT_SCRAP -> AlterantItems.SEALANT_SCRAP.get();
            case LATTICE_FRAGMENTS -> AlterantItems.LATTICE_FRAGMENTS.get();
            case CHRONAL_DROSS -> AlterantItems.CHRONAL_DROSS.get();
        };
    }
