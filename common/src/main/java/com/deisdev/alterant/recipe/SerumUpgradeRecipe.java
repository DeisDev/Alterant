...
public final class SerumUpgradeRecipe extends NormalCraftingRecipe {
    ...
    @Override protected PlacementInfo createPlacementInfo() {
        ...
        ingredients.add(Ingredient.of(AlterantItems.compound(inputSerum)));
        ingredients.addAll(additives);
        return PlacementInfo.create(ingredients);
    }
    @Override public boolean matches(CraftingInput input, Level level) { return valid(input); }
    @Override public ItemStack assemble(CraftingInput input) { return valid(input) ? AlterantItems.compound(resultSerum).getDefaultInstance() : ItemStack.EMPTY; }
    ...
    @Override public List<RecipeDisplay> display() {
        ...
        inputs.add(new SlotDisplay.ItemStackSlotDisplay(ItemStackTemplate.fromNonEmptyStack(AlterantItems.compound(inputSerum).getDefaultInstance())));
        additives.forEach(ingredient -> inputs.add(ingredient.display()));
        return List.of(new ShapelessCraftingRecipeDisplay(inputs,
                new SlotDisplay.ItemStackSlotDisplay(ItemStackTemplate.fromNonEmptyStack(AlterantItems.compound(resultSerum).getDefaultInstance())),
