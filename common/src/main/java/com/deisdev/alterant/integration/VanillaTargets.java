...
final class VanillaTargets {
    ...
    static List<BlockPos> resolve(PreservationContext context) {
        ...
        if (state.getBlock() instanceof ChestBlock && state.getValue(ChestBlock.TYPE) != ChestType.SINGLE) {
            ...
            if (!context.level().hasChunkAt(otherPos)) { throw new IllegalArgumentException("Load both halves of the chest first"); }
            ...
            if (!other.is(state.getBlock()) || other.getValue(ChestBlock.TYPE) == ChestType.SINGLE
                ...
                throw new IllegalArgumentException("Chest connection is incomplete");
            }
