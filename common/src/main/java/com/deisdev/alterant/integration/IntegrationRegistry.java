...
public final class IntegrationRegistry {
    ...
    private static boolean selects(PreservationAdapter adapter, PreservationContext context) {
        return context.formulation() == com.deisdev.alterant.api.Formulation.TRANSFER_SEAL
    ...
    public static synchronized void register(PreservationAdapter adapter) {
        if (locked) { throw new IllegalStateException("Register Alterant adapters during mod initialization, before the first server starts"); }
    ...
    public static synchronized void registerPermission(PreservationPermission permission) {
        if (locked) { throw new IllegalStateException("Register Alterant permissions during mod initialization"); }
    ...
    public static void observed(PreservationContext context, List<AdapterSnapshot> snapshots, boolean paused) {
        for (var snapshot : snapshots) {
            ...
            } catch (RuntimeException error) { Constants.LOG.error("Alterant adapter observation failed: {}", snapshot.id(), error); }
        }
    }
