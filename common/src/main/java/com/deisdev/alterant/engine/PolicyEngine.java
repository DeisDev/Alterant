...
public final class PolicyEngine {
    ...
    public static boolean blocksProperty(Level level, BlockPos treated, Action action, BlockPos source, BlockPos target, String property) {
        var record = ((PreservationLevel) level).alterant$treatments().get(treated.asLong());
        if (record == null || !record.actions().contains(action)) { return false; }
        return protects(level, record, action, source, target, property) && (record.formulation() != com.deisdev.alterant.api.Formulation.GROWTH_REGULATOR
