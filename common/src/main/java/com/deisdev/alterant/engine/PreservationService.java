...
public final class PreservationService {
    ...
    public PreservationService(ServerLevel level) {
        ...
        var file = TreatmentStore.TYPE.id().withSuffix(".dat")
                .resolveAgainst(((SavedDataStorageAccessor) storage).alterant$dataFolder());
        ...
        if (loaded == null && Files.exists(file)) {
            throw new IllegalStateException("Cannot read Alterant treatments at " + file
                    + ". Restore a backup or use the matching Alterant version; the file was not overwritten.");
        }
        store = loaded == null ? storage.computeIfAbsent(TreatmentStore.TYPE) : loaded;
        deferred = new DeferredTickStore(level, store);
        ((PreservationLevel) level).alterant$setTreatments(store);
        scheduler(false).alterant$bind(this, false);
        scheduler(true).alterant$bind(this, true);
    }

    public static PreservationService get(ServerLevel level) {
        return java.util.Objects.requireNonNull(((PreservationLevel) level).alterant$service(), "Alterant level not initialized");
    }
    ...
    private void acceleratePending(BlockPos pos) {
        for (var pending : this.<Block>scheduler(false).alterant$take(pos)) {
            // Alterant type/position, priority and order; the insertion hook scales the remaining delay once.
    ...
    public Result scrape(BlockPos pos, net.minecraft.server.level.ServerPlayer player) {
        ...
        var access = new PlayerAccess(player, () -> player.getMainHandItem().is(com.deisdev.alterant.item.AlterantItems.SCRAPER.get())
    ...
    Result dissolveArea(BlockPos pos, net.minecraft.server.level.ServerPlayer player, com.deisdev.alterant.item.SolventCharge cost, int limit) {
    ...
    private Result remove(BlockPos pos, com.deisdev.alterant.integration.OperationAccess access, RemovalCause cause, net.minecraft.server.level.ServerPlayer collector,
        ...
        try {
            ...
            com.deisdev.alterant.item.SolventCharge.Prepared payment = null;
            try {
    ...
    private void afterCommit(BlockPos pos, Runnable notification) {
        ...
        catch (RuntimeException error) { com.deisdev.alterant.Constants.LOG.error("Alterant removal committed at {}, but notification failed", pos, error); }
    }
    ...
    public void capturePending(BlockPos pos) {
        ...
        if (treatment.actions().contains(Action.SCHEDULED_BLOCK_TICK)) {
            for (var pending : this.<Object>scheduler(false).alterant$take(pos)) { retain(pending.tick(), false, pending.collected()); }
        }
        if (treatment.actions().contains(Action.SCHEDULED_FLUID_TICK)) {
            for (var pending : this.<Object>scheduler(true).alterant$take(pos)) { retain(pending.tick(), true, pending.collected()); }
        }
    }
    ...
    private void checkThread() {
        if (!level.getServer().isSameThread()) { throw new IllegalStateException("Alterant mutations require the server thread"); }
    }
