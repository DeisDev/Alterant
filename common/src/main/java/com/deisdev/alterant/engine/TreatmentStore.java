...
public final class TreatmentStore extends SavedData {
    ...
    public static final SavedDataType<TreatmentStore> TYPE = new SavedDataType<>(
            Identifier.fromNamespaceAndPath("alterant", "treatments"), TreatmentStore::new, CODEC, DataFixTypes.LEVEL);
    ...
    private static DataResult<TreatmentStore> decode(Payload payload) {
        if (payload.schema != SCHEMA) {
            return DataResult.error(() -> "Unsupported Alterant save schema " + payload.schema + "; expected " + SCHEMA);
        }
        ...
        for (Treatment treatment : payload.records) {
            ...
            if (store.get(treatment.position()) != null) {
                return DataResult.error(() -> "Duplicate Alterant position " + treatment.position());
            }
            store.put(treatment);
        }
        for (ResumingTicks work : payload.resuming) {
            if (store.resuming(work.position()) != null || store.get(work.position()) != null) {
                return DataResult.error(() -> "Duplicate or conflicting Alterant resumption " + work.position());
            }
        ...
        for (var mask : payload.masks) {
            if (store.mask(mask.position()) != null || store.chunkMaskSize(chunkKey(mask.position())) >= 4096) {
                return DataResult.error(() -> "Duplicate or excessive Alterant masks");
            }
            store.putMask(mask);
        }
