...
public final class SerumStatusQueries {
    ...
    public static Optional<SerumStatusPayload> query(ServerPlayer player, SerumStatusRequest request) {
        ...
        var effect = treatment == null || !treatment.blockId().equals(block) ? Optional.<com.deisdev.alterant.engine.Acceleration>empty() : treatment.acceleration();
        return Optional.of(new SerumStatusPayload(request.request(), request.dimension(), request.position(), block,
                effect.isPresent() ? treatment.formulation().ordinal() : -1, effect.map(com.deisdev.alterant.engine.Acceleration::multiplier).orElse(1.0),
                effect.map(com.deisdev.alterant.engine.Acceleration::remainingTicks).orElse(0),
